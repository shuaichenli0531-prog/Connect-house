export default function ProgramCard({ item, applyLabel = "Apply", lang = "en" }) {
  ...
  return (
    <CardWrapper>
      <div className="card group overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl cursor-pointer">
        {item.imageUrl && (
        ...
        <div className="p-6">
          ...
          <div className="mt-4 flex items-center justify-between text-xs uppercase tracking-[0.2em] text-white/50">
            ...
            <span className="text-gold transition-all group-hover:translate-x-1">
              {applyLabel} →
            </span>
          </div>
        </div>
